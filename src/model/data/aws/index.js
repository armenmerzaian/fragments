...
// Create one in-memory database for fragment metadata (raw data handled by s3, for now)
...
async function readFragment(ownerId, id) {
  ...
  logger.debug({ params }, 'Reading fragment from DynamoDB');
...
async function listFragments(ownerId, expand = false) {
  ...
  logger.debug({ params }, 'Listing fragments from DynamoDB');
...
async function deleteFragment(ownerId, id) {
  ...
  logger.debug({ s3Params }, 'Deleting fragment data from S3');
  ...
  logger.debug({ dynamoParams }, 'Deleting fragment data from dynamoDB');
  ...
  try {
    // Use our client to send the S3 delete command
  ...
  try {
    // Use our client to send the DynamoDB delete command
